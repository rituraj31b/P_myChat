...
function App() {
  ...
  return (
    <>
      <h1 className="main_heading"><i>myChat</i></h1>
      <h5 className="my_socket"><i>Me : {socketId}</i></h5>
      ...
      {!joinedRoom && (
        <div className="container">
          <div className="users-container">
            <h2 className="users_heading">other Users:</h2>
            <ul className="users">
              {users.map((user) => {
                return (
                  <li className="user" key={user}>
                    {user && user === socketId ? `you` : user}
                  </li>
                );
              })}
            </ul>
          ...
          <div className="rooms-container">
            <p className="rooms_heading">Available Rooms Now:</p>

            {rooms.length === 0 ? (
              <p className="no_rooms">No Rooms Present! Create a new room !</p>
            ) : (
              <ul className="rooms">
                {rooms.map((room) => {
                  return (
